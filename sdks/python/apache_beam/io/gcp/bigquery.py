...
bigquery_v2_messages.TableSchema`. or a `ValueProvider` that has a JSON string,
      ...
      triggering_frequency (int): Every triggering_frequency duration (expressed
        in seconds), a BigQuery load job will be triggered for all the data
        written since the last load job. BigQuery has limits on how many
        load jobs can be triggered per day, so be careful not to set this
        duration too low, or you may exceed daily quota. Often this is set
        to 5 or 10 minutes to ensure that the project stays well under the
        BigQuery quota.
